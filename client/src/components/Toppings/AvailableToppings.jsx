...
const AvailableToppings = props => {
  const [amountState, setAmountState] = useState(props.amount);


  const onSaveItem = enteredAmount => {
    ...
    // console.log('selectedItemData 속의 amount: ', selectedItemData.amount)
  ...
  const toppingsList = filteredCategory[0].DUMMY_TOPPINGS.map(topping => 
    <React.Fragment key={topping.id}>
      ...
      />
      {topping.amount}
      <ToppingItemForm onChange={e => setAmountState(e.target.value)} key={topping.id} amount={topping.amount} onSaveItem={onSaveItem} id={topping.id} amountState={amountState} />
    </React.Fragment>
  )
